...
public class TrueTypeFont {
    ...
    public static Font getFont(String fontname, int style, float size) {
        ...
        for (final Font font : graphicsenvironment.getAllFonts()) {
    ...
    private class IntObject {
        int storedX; // Character's stored x position
        int storedY; // Character's stored y position
        int width; // Character's width
    ...
    private void drawQuad(float drawX, float drawY, float drawX2, float drawY2, float srcX, float srcY, float srcX2, float srcY2) {
        ...
        /*
         * GL11.glTexCoord2f(TextureSrcX, TextureSrcY); GL11.glVertex2f(drawX, drawY);
         * 
         * GL11.glTexCoord2f(TextureSrcX, TextureSrcY + RenderHeight); GL11.glVertex2f(drawX, drawY + DrawHeight);
         * 
         * GL11.glTexCoord2f(TextureSrcX + RenderWidth, TextureSrcY + RenderHeight); GL11.glVertex2f(drawX + DrawWidth, drawY + DrawHeight);
         * 
         * GL11.glTexCoord2f(TextureSrcX + RenderWidth, TextureSrcY); GL11.glVertex2f(drawX + DrawWidth, drawY);
         */
    ...
    public void drawString(float x, float y, String whatchars, int startIndex, int endIndex, float scaleX, float scaleY, int format) {
        ...
        // int bufferSi
        final FloatBuffer verticesBuffer = BufferUtils.createFloatBuffer((endIndex - startIndex) * vectorSite * 4);
        ...
        while (i >= startIndex && i <= endIndex) {
            ...
            if (intObject != null) {
                ...
                if (charCurrent == '\n') {
                    ...
                    drawQuad((totalwidth + intObject.width) * scaleX + x, startY * scaleY + y, totalwidth * scaleX + x, (startY + intObject.height) * scaleY
                            + y,

                    intObject.storedX + intObject.width, intObject.storedY + intObject.height, intObject.storedX, intObject.storedY);
    ...
    public static int loadImage(BufferedImage bufferedImage) {
        try {
            ...
            final int internalFormat = GL11.GL_RGBA8, format = GL11.GL_RGBA;
