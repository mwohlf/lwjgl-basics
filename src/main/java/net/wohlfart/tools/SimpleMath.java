...
public final class SimpleMath {
	...
	public static Quaternion createQuaternion(final Vector3f start, final Vector3f end, final Quaternion result) {
		...
		Vector3f startNorm = start.normalise(new Vector3f());
		Vector3f endNorm = end.normalise(new Vector3f());

		float dot = Vector3f.dot(startNorm, endNorm);
		...
		float invs = 1 / s;
		Vector3f c = Vector3f.cross(startNorm, endNorm, new Vector3f());
