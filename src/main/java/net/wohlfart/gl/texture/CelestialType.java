...
public enum CelestialType {
	...
	GAS_PLANET {
		...
		ColorGradient gradient = new ColorGradient(new Color(255, 213, 133),
				new Color(102, 68, 58));
	 ...
	 * @return the color on the surface at the location with the specified
	 *         normal vector
	 */
	...
	// adding octaves
	double createNoise(final float x, final float y, final float z,
			final float v, final float persistence, final int octaves) {
		double result = 0;
		float max = 0;
	...
	// calling the noise
	double createNoise(final float x, final float y, final float z,
			final float v, final float amplitude, final float frequency) {
