...
/**
 * @formatter:off - the projection matrix defines the lens of the camera it translates the world space into 2D screen space
 * 
 *                - the view matrix defines the position and the direction of the camera it is set once per rendering pass and defined in which direction the
 *                cam is looking
 * 
 *                - the model matrix defines the position and direction of each 3D model it is used to move and rotate a model in the world space around each
 *                model can set its individual matrix before rendering so it is set for each model object
 * 
