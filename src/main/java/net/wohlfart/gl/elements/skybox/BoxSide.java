...
enum BoxSide {
	...
	protected int createAndLoadTexture(int textureUnit,
		...
		int width = parameters.getSize();
		int height = parameters.getSize();
	...
	protected int[] createClouds(int[] data, int width, int height, PerlinNoiseParameters param) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				double noise = createNoise(vector.x * frequency, vector.y * frequency, vector.z * frequency, w * frequency, persistence, octaves);
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				if ((noise > -0.0001) && (noise < +0.0001)) {
					Color color = gradient.getColor(noise * 1000);
					data[x + y * width] = color.getRGB() << 8;
				}
			}
		}
	...
	protected double createNoise(final float x, final float y, final float z,
			final float w, final float persistence, final int octaves) {
		double result = 0;
		float max = 0;
	...
	protected double createNoise(final float x, final float y, final float z,
			final float w, final float amplitude, final float frequency) {
